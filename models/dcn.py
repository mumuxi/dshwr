...
class DCNLoss(torch.nn.Module):
    def __init__(self, lamda, eta, beta, cl):
        super(DCNLoss, self).__init__()
        self.lamda = lamda
        self.eta = eta
        self.beta = beta
        self.cl = cl

    def forward(self, U_cnn, S, U=None, noise=None):
        ...
        ip_1 = tf.matmul(u, tf.transpose(v))
            ...
            mod_1 = tf.sqrt(tf.matmul(reduce_shaper(tf.square(u)), reduce_shaper(
                tf.square(v)) + tf.constant(0.000001), transpose_b=Tr))
        ...
        if U is None:
            U = U_cnn
        ip_1 = U_cnn @ U.t()
        mod_1 = torch.sqrt(torch.sum(U_cnn * U_cnn, dim=-1, keepdim=True) @ torch.sum(U * U, dim=-1, keepdim=True).t())
        theta = self.cl / 2.0 * (-torch.div(ip_1, mod_1) + 1.0)
        ...
        masks[S <= 0] = S12 / S0
        # Prevent overflow
        theta = torch.clamp(theta, min=0.00001)
        pair_loss = ((torch.log(self.eta / theta + 1.0) + S * torch.log(theta / self.eta)) * masks).mean()
        regular_term = torch.log(self.cl / (2.0 * self.eta) * (-torch.div(torch.sum(U_cnn, dim=-1), torch.sqrt(torch.sum(U_cnn * U_cnn, dim=-1) * self.cl)) + 1.0) + 1.0).mean()
        if noise is not None:
            loss = pair_loss + self.lamda * regular_term - self.beta * U_cnn.mul(noise).mean()
        else:
            loss = pair_loss + self.lamda * regular_term
...
def train(train_dataloader, query_dataloader, retrieval_dataloader, arch, code_length, device, lr, max_iter,
              lamda, eta, beta, b_reg, topk, evaluate_interval, pre_train):
    ### create model
    ...
    '''
    criterion = DCNLoss(lamda, eta, beta, code_length)
    ...
    for epoch in iteration:
        ...
        for data, targets, index in train_dataloader:
            ...
            loss = criterion(U_cnn, S, noise=noises)
